...
export const Functions = {
  IncrementalAddPicturesBulk: async (knex: Knex, paths: string[]): Promise<void> => {
    ...
    const pictureRows = paths.map((path) => {
      ...
      return {
        folder,
        path,
        sortKey: Imports.Helpers.ToSortKey(posix.basename(path)),
        pathHash: createHash('sha512').update(path).digest('base64'),
      }
    })
    await Imports.Helpers.ExecChunksSynchronously(
      Imports.Helpers.Chunk(pictureRows, Imports.getDbChunkSize(knex)),
      async (chunk) => {
  ...
  IncrementalEnsureFoldersBulk: async (knex: Knex, folderPaths: string[]): Promise<void> => {
    ...
    const rows = [...unique].map((folderPath) => {
      ...
      return {
        folder: parent,
        path: folderPath,
        sortKey: Imports.Helpers.ToSortKey(posix.basename(withoutSlash)),
      }
    })
    await Imports.Helpers.ExecChunksSynchronously(
      Imports.Helpers.Chunk(rows, Imports.getDbChunkSize(knex)),
      async (chunk) => {
  ...
  IncrementalRemovePicturesBulk: async (knex: Knex, paths: string[]): Promise<void> => {
    if (paths.length === ZERO) return
    await Imports.Helpers.ExecChunksSynchronously(
      Imports.Helpers.Chunk(paths, Imports.getDbChunkSize(knex)),
      async (chunk) => {
  ...
  IncrementalEnsureAncestors: async (logger: Debugger, knex: Knex, affectedFolders: Set<string>): Promise<void> => {
    ...
    for (const folder of affectedFolders) {
      ...
      Imports.Helpers.AddFolderAndAncestors(ancestors, parentPath)
    }
    ...
    await Imports.Helpers.ExecChunksSynchronously(
      Imports.Helpers.Chunk(ancestorList, Imports.getDbChunkSize(knex)),
      async (chunk: string[]) => {
    ...
    const rows = missing.map((path) => {
      ...
      const sortKey = Imports.Helpers.ToSortKey(posix.basename(withoutSlash))
  ...
  IncrementalUpdateFolders: async (logger: Debugger, knex: Knex, affectedFolders: Set<string>): Promise<void> => {
    ...
    if (folderList.length > ZERO) {
      ...
      await Imports.Helpers.ExecChunksSynchronously(
        Imports.Helpers.Chunk(folderList, Imports.getDbChunkSize(knex)),
        async (chunk: string[]) => {
      ...
      const updateRows = folderList.map((path) => {
        ...
        const sortKey = path === posix.sep ? '' : Imports.Helpers.ToSortKey(posix.basename(withoutSlash))
        return { path, totalCount, seenCount, folder: parent, sortKey }
      })
      await Imports.Helpers.ExecChunksSynchronously(
        Imports.Helpers.Chunk(updateRows, Imports.getDbChunkSize(knex)),
        async (chunk) => {
  ...
  IncrementalUpdateFirstImages: async (logger: Debugger, knex: Knex): Promise<void> => {
    await Imports.FolderFunctions.SyncFolderFirstImages(logger, knex)
  },
  IncrementalSync: async (knex: Knex, changeset: Changeset, dataDir = '/data'): Promise<void> => {
    ...
    for (const dirPath of dirDeletes) {
      Imports.Helpers.AddFolderAndAncestors(affectedFolders, dirPath)
    ...
    for (const path of fileDeletes) {
      ...
      Imports.Helpers.AddFolderAndAncestors(affectedFolders, folder)
    }
