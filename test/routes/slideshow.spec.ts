...
export class SlideshowGetUnreadImageCount {
  ...
  async 'it should select only where path matches'(): Promise<void> {
    ...
    expect(this.KnexInstance.where.firstCall.args).to.deep.equal(['path', 'like', `${target}%`])
  }
  ...
  async 'it should select only unread images'(): Promise<void> {
    ...
    expect(this.KnexInstance.andWhere.firstCall.args).to.deep.equal(['seen', '=', false])
  }
...
export class SlideshowGetImagesTests {
  ...
  async 'it should extract paths from results'(): Promise<void> {
    ...
    const result = ['/foo/image.txt', '/foo/spreadhseet.png', '/foo/program.mp4', '/foo/movie.xls', '/foo/webpage.exe']
...
export class SlidewhowMarkImageReadTests {
  ...
  KnexStub = sinon
    .stub()
    .onFirstCall()
    .returns(this.KnexFirstInstance)
    .onSecondCall()
    .returns(this.KnexSecondInstance)
    .onThirdCall()
  ...
  async 'it should increment seen counts for all parent folders'(): Promise<void> {
    ...
    expect(this.KnexSecondInstance.whereIn.firstCall.args).to.deep.equal(['path', ['/foo/bar/', '/foo/', '/']])
  }
...
export class SlideshowTickCountdownTests {
  ...
  IoFake = Cast<WebSocketServer>(this.IoStub)
...
export class SlideshowHandleSocketTests {
  ...
  SocketFake = Cast<WebSocket>(this.SocketStub)
  ...
  async joinSlideshow(roomname: string): Promise<void> {
    const fn = this.SocketStub.on
      ...
      .map((call) => Cast<undefined | ((o: string) => Promise<void>)>(call.args[1]))[0]
    assert(fn !== undefined)
  ...
  async 'on(get-launchId) it should reply with current launchId'(): Promise<void> {
    ...
    const fn = this.SocketStub.on
      ...
      .map((call) => Cast<undefined | ((o: Sinon.SinonStub) => Promise<void>)>(call.args[1]))[0]
    assert(fn !== undefined)
    expect(fn).to.be.a('function')
  ...
  async 'on(prev-image) it should ignore call when not joined to room'(): Promise<void> {
    ...
    const fn = this.SocketStub.on
      ...
      .map((call) => Cast<undefined | (() => Promise<void>)>(call.args[1]))[0]
    assert(fn !== undefined)
    expect(fn).to.be.a('function')
    await fn()
    expect(this.GetRoomStub?.callCount).to.equal(0)
...
export class SlideshowGetRouterTests {
  ...
  AppFake = Cast<Application>({ App: Math.random() })
  ...
  SocketFake = Cast<WebSocket>({ Socket: Math.random() })

  AppRouterStub = {
    get: sinon.stub(),
  }

  RequestStub: FakeRequest = {
    params: [] as string[],
  }

  ResponseStub: FakeResponse = {
  ...
  before(): void {
    ...
    this.RouterStub = sinon.stub(Imports, 'Router').returns(Cast<Router>(this.AppRouterStub))
  ...
  async 'it should reply to /launchId with the launch id'(): Promise<void> {
    ...
    await handler(Cast<Request>(this.RequestStub), Cast<Response>(this.ResponseStub))
  ...
  async 'it should tolerate handler for route / rejecting'(): Promise<void> {
    ...
    const fn = Cast<RequestHandler>(this.AppRouterStub.get.secondCall.args[1])
    const spy = sinon.stub(Functions, 'RootRoute').rejects('FOO')
    try {
      await fn(Cast<Request>(null), Cast<Response>(null))
    } finally {
      spy.restore()
    }
  }
  ...
  async 'it should tolerate handler for route /* rejecting'(): Promise<void> {
    ...
    const fn = Cast<RequestHandler>(this.AppRouterStub.get.thirdCall.args[1])
    const spy = sinon.stub(Functions, 'RootRoute').rejects('FOO')
    try {
      await fn(Cast<Request>(null), Cast<Response>(null))
    } finally {
  ...
  async 'it should handle web socket connections'(): Promise<void> {
    ...
    Cast<(o: unknown) => void>(this.IoStub.on.firstCall.args[1])(this.SocketFake)
  ...
  async 'it should set interval for changing images'(): Promise<void> {
    ...
    await Cast<() => Promise<void>>(this.setIntervalStub?.firstCall.args[0])()
