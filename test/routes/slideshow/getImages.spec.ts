...
describe('routes/slideshow getImages()', () => {
  ...
  describe('when called with page=0 and limit=40', () => {
    ...
    it('should select only path column', () => {
      expect(knexInstanceStub.select.mock.calls[0]).toEqual(['path'])
    })
    ...
    it('should order by first and second order sorts', () => {
      expect(knexInstanceStub.orderBy.mock.calls.length).toBe(2)
    })
    it('should sort first by seen flag', () => {
      expect(knexInstanceStub.orderBy.mock.calls[0]).toEqual(['seen'])
    })
    it('should sort second by pathHash column', () => {
      expect(knexInstanceStub.orderBy.mock.calls[1]).toEqual(['pathHash'])
    })
    it('should offset into results', () => {
      expect(knexInstanceStub.offset.mock.calls.length).toBe(1)
    })
    it('should limit results', () => {
      expect(knexInstanceStub.limit.mock.calls.length).toBe(1)
    })
    it('should limit results to expected limit', () => {
      expect(knexInstanceStub.limit.mock.calls[0]).toEqual([40])
    })
  ...
  describe('when called with page=3', () => {
    it('should offset by expected amount', async () => {
      ...
      expect(knexInstanceStub.offset.mock.calls[0]).toEqual([120])
    })
  })
