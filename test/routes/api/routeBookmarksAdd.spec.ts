...
describe('routes/api route POST /bookmarks/add', () => {
  ...
  let addBookmarkStub: MockInstance = vi.fn()
  ...
  beforeEach(async () => {
    ...
    addBookmarkStub = vi.spyOn(Imports, 'addBookmark').mockResolvedValue(undefined)
    ...
    routeHandler = cast(
      postFn.mock.calls.find((call) => call[0] === '/bookmarks/add')?.[1],
  ...
  it('should log on entry to bookmarks/add handler', async () => {
    await routeHandler(requestFake, responseFake)
    const matched = loggerStub.mock.calls.some((c) => String(c[0]).includes('POST /bookmarks/add'))
