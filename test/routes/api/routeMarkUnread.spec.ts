...
describe('routes/api route POST /mark/unread', () => {
  ...
  beforeEach(async () => {
    ...
    routeHandler = cast(
      postFn.mock.calls.find((call) => call[0] === '/mark/unread')?.[1],
  ...
  it('should call markFolderSeen with markAsSeen false', async () => {
    requestStub.body.path = 'foo/a%20bar/baz'
    await routeHandler(requestFake, responseFake)
    expect(markFolderSeenStub.mock.calls[0]?.[2]).toBe(false)
  })
  ...
  it('should log on entry to mark/unread handler', async () => {
    await routeHandler(requestFake, responseFake)
    const matched = loggerStub.mock.calls.some((c) => String(c[0]).includes('POST /mark/unread'))
