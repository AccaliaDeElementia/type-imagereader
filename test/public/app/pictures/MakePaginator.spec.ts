...
describe('public/app/pictures function MakePaginator()', () => {
  ...
  beforeEach(() => {
    ...
    mountDom(dom)
    makePageItemSpy = sandbox.stub(Grid, 'MakePaginatorItem').callsFake(() => dom.window.document.createElement('li'))
    getCurrentPageSpy = sandbox.stub(Grid, 'GetCurrentPage').returns(0)
    resetPubSub()
  })
  afterEach(() => {
    sandbox.restore()
  ...
  it('should return null for negative page count', () => {
    const result = Grid.MakePaginator(-7)
    expect(result).to.equal(null)
  })
  it('should return null for zero page count', () => {
    const result = Grid.MakePaginator(0)
    expect(result).to.equal(null)
  })
