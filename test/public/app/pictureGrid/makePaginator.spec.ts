...
import { Internals } from '#public/scripts/app/pictureGrid.js'
...
describe('public/app/pictures makePaginator()', () => {
  ...
  it('should create previousPage element first', () => {
    ...
    expect(makePageItemSpy.firstCall.args[0]).toBe('«')
  })
  ...
  it('should create nextPage element last', () => {
    ...
    Internals.makePaginator(7)
    expect(makePageItemSpy.getCall(8).args[0]).toBe('»')
  })
  ...
  it('should create specific page elements with correct labels', () => {
    Internals.makePaginator(7)
    for (let i = 1; i <= 7; i += 1) {
      expect(makePageItemSpy.getCall(i).args[0]).toBe(`${i}`)
    }
  })
