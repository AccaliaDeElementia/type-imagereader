...
describe('sync/synchronize function synchronize()', () => {
  ...
  beforeEach(() => {
    ...
    updateFolderPictureCountsStub = sandbox.stub(Imports.FolderCounts, 'UpdateFolderPictureCounts').resolves()
    pruneEmptyFoldersStub = sandbox.stub(Imports.FolderCounts, 'PruneEmptyFolders').resolves()
    emitSqliteSizeWarningStub = sandbox.stub(Imports.SqliteWarning, 'EmitSqliteSizeWarning')
  })
  afterEach(() => {
    sandbox.restore()
  })
